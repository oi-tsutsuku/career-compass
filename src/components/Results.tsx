...
const AXES: Axis[] = ['depth', 'self', 'social', 'action', 'decision']
...
export default function Results() {
  ...
  return (
    <div className="page">
      ...
      {/* ── Section 1: Pattern reveal ─────────────────────────── */}
      <div style={{ width: '100%', background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        <div className="container" style={{ paddingTop: 48, paddingBottom: 48 }}>
          <div className="fade-in">
            <span className="caption" style={{ color: 'var(--text-3)', display: 'block', marginBottom: 16 }}>
              あなたの就活タイプ
            </span>
            ...
            <p style={{ fontSize: '0.9375rem', lineHeight: 1.75, color: 'var(--text-2)', marginBottom: 16 }}>
              {hl}
            </p>
      ...
      <div style={{ width: '100%', background: 'var(--bg)', padding: '48px 0' }}>
        <div className="container">
          <div className="depth-hero fade-up">
            ...
            <div className="score-number">{scores.depth}</div>
            <div style={{ fontSize: '1rem', opacity: 0.8, marginTop: 8, marginBottom: 20 }}>
              / 100 — {depthLevel}
            </div>
            <p style={{ fontSize: '0.9375rem', lineHeight: 1.75, opacity: 0.85, maxWidth: 480, margin: '0 auto' }}>
              ...
              スコアが高いほど、少ない経験でも面接で説得力を持って話せる可能性があります。
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="container">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {AXES.map((axis) => {
              ...
              return (
                <div key={axis} className={`fade-up${isDepth ? '' : ''}`}
                  style={{
                  ...
                  <div style={{
                    height: 6,
                    background: isDepth ? 'rgba(255,255,255,0.2)' : 'var(--border)',
                    borderRadius: 99,
                    overflow: 'hidden',
                  }}>
                    <div style={{
                      height: '100%',
                      width: `${score}%`,
                      background: isDepth ? '#fff' : meta.color,
                      borderRadius: 99,
        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gap: 24, gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', marginBottom: 40 }}>
            ...
            <div className="card" style={{ background: 'var(--accent-bg)', borderColor: 'rgba(37,99,235,0.15)' }}>
              ...
              <p style={{ fontSize: '0.9375rem', lineHeight: 1.75, color: 'var(--text-2)' }}>
                {pattern.tip}
              </p>
            </div>
          </div>
      ...
      <div style={{ width: '100%', padding: '48px 0', background: 'var(--bg)', borderTop: '1px solid var(--border)' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <p className="body" style={{ marginBottom: 32, maxWidth: 480, margin: '0 auto 32px' }}>
            ...
            完了後に個別フィードバックを受け取れます。
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center' }}>
            <button
              className="btn btn--primary"
              style={{ minWidth: 280, height: 52 }}
              onClick={() => dispatch({ type: 'GO', screen: 'detailQuiz' })}
            >
              詳細診断に進む（25問）
            </button>
            <button
