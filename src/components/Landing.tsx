      ...
      <div className="container" style={{ paddingTop: 64, paddingBottom: 80 }}>
        ...
        <div className="fade-up fade-up-3" style={{ marginBottom: 56 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {[
              { label: '自己理解', color: '#7C3AED' },
              { label: '社会探索', color: '#059669' },
              { label: '行動性',   color: '#D97706' },
              { label: '意思決定', color: '#DC2626' },
              { label: '深度・解像度', color: 'var(--depth)', weight: true },
            ].map(({ label, color, weight }) => (
              <span key={label} style={{
                padding: '5px 14px',
                borderRadius: 99,
                fontSize: '0.8125rem',
                fontWeight: weight ? 700 : 500,
                color: weight ? '#fff' : color,
                background: weight ? 'var(--depth)' : 'transparent',
                border: weight ? 'none' : `1.5px solid ${color}`,
              }}>
                {label}
              </span>
            ))}
