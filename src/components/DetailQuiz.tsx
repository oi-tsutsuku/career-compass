        ...
        <div className="fade-in" key={current}>
          ...
          {q.type === 'ab' ? (
            <div className="ab-grid">
              <button className={`ab-btn${selected === 1 ? ' selected' : ''}`} onClick={() => select(1)}>{q.a}</button>
              <button className={`ab-btn${selected === 2 ? ' selected' : ''}`} onClick={() => select(2)}>{q.b}</button>
            </div>
          ) : (
            <div>
              <div className="scale-row" style={{ marginBottom: 12 }}>
                {[1,2,3,4,5].map(val => (
                  <button key={val} className={`scale-btn${selected === val ? ' selected' : ''}`} onClick={() => select(val)}>{val}</button>
                ))}
              </div>
