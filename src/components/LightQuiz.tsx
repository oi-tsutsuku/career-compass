...
const AXIS_LABEL: Record<string, string> = {
  self: '自己理解', social: '社会探索', action: '行動性', decision: '意思決定', depth: '深度・解像度',
}
      ...
      <div style={{ width: '100%', background: 'var(--surface)', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '14px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <span className="small" style={{ color: 'var(--text-3)' }}>{current + 1} / {total}</span>
          </div>
        ...
        <div className="fade-in" key={current}>
          ...
          <div className="answer-grid">
            {SCALE_LABELS.map((label, i) => {
              const val = i + 1
              return (
                <button key={val} className={`answer-btn${selected === val ? ' selected' : ''}`} onClick={() => select(val)}>
                  <span className="dot" />
                  <span>{label}</span>
                </button>
              )
